//
// A complete and working MCP agent that provides a 'calculator' tool.
...
// A simple calculator tool handler
...
public:
    static mcp::json handle(const mcp::json& params, const std::string& /* session_id */) {
        ...
        std::string expr_str = clean_llm_response(params["expression"]);
        if (expr_str.empty()) {
            throw mcp::mcp_exception(mcp::error_code::invalid_params, "Empty expression provided");
        }

        try {
            double result = evaluate_expression(expr_str);
            return {
                { "type", "text" },
...
private:
    static double evaluate_expression(const std::string& expr) {
        ...
        char op;
        if (!(iss >> num1 >> op >> num2)) {
            throw std::runtime_error("Invalid expression format. Expected 'number operator number'.");
        }
        ...
        switch (op) {
            ...
            case '/':
                ...
                return num1 / num2;
            default:
                throw std::runtime_error("Invalid operator. Only +, -, *, / are supported.");
        }
    }
};
...
struct Config {
    ...
    int n_ctx = 2048;
    int n_batch = 512;
    std::string system_prompt = "You are a helpful assistant with access to a calculator. When the user asks for a calculation, you must first think about the expression to evaluate. Then, you must call the 'calculator' tool with that expression. After you receive the result, you must formulate a final answer to the user based on the tool's output.";
};
...
int main(int argc, char ** argv) {
    ...
    server.set_server_info("CalculatorAgentServer", "0.1.0");
    server.set_capabilities({{"tools", json::object()}});
    mcp::tool calc_tool = mcp::tool_builder("calculator")
        .with_description("Perform a basic arithmetic calculation.")
        .with_string_param("expression", "The mathematical expression to evaluate (e.g., '10 + 5').")
        .build();
    ...
    if (!client.initialize("CalculatorAgentClient", "0.1.0")) { fprintf(stderr, "Error: Failed to initialize MCP client.\n"); return 1; }

    // --- AGENT INITIALIZATION ---
    std::vector<common_chat_tool> llm_tools = {{calc_tool.name, calc_tool.description, calc_tool.parameters_schema.dump()}};
    ...
    printf("Calculator MCP Agent is running. Press Ctrl+C to exit.\n");
    std::string line;
    while (true) {
        printf("\n> ");
